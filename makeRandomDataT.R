...
makeRandomData <- function(n,
                           ...
                           minG0 = 1e-3,
                           minR2 = 0.01, maxR2 = 0.99, pos = .001, skewing = c(-1,1),
                           ...) {
  ...
  # make sure 99% of all obs are within positivity tolerance
  tol = TRUE
  its = 0

  while (tol & its < 20) {
    tol = mean(plogis(.8^its*logitg0 + skewage) < minG0) > 0.01 |
      mean(plogis(.8^its*logitg0 + skewage) > (1 - minG0)) >
      0.01
    its = its + 1
  }
